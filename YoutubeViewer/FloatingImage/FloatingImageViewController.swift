...
//        let tabBarHeight = tabBarController?.tabBar.frame.height ?? 0.0
        ...
        videoImageView.center.x = maxX - (newWidth / 2)
        videoImageView.center.y = maxY - (newHeight / 2)
        videoImageView.contentMode = .redraw
//        videoImageView.bounds.size = CGSize(width: newWidth, height: newHeight)
